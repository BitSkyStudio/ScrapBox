...
public class ReceiverGameObject extends GameObject {
    public int channel;
    public ReceiverGameObject(Vector2 position, float rotation, Server server, GameObjectConfig config) {
        ...
        shape.setAsBox(0.55f, 0.75f);
        fixtureDef.shape = shape;
        fixtureDef.density = 1F;
        base.createFixture(fixtureDef);
        this.setBody("base", "receiver", base);
    ...
    public ArrayList<EditorUIRow> createEditorUI() {
        ...
        row.add(new EditorUILabel("received: "));
    ...
    public float getValueOnOutput(int id) {
        return server.backCommunications.getOrDefault(channel, 0f);
    }

    @Override
    public HashMap<String, ConnectionEdge> getConnectionEdges() {
        HashMap<String, ConnectionEdge> edges = new HashMap<>();
        edges.put("down", new ConnectionEdge(new Vector2(0, -0.75f), ConnectionEdgeType.Normal));
        return edges;
    }
}
